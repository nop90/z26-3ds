...
void srv_SetPalette()
{
	...
	for ( i=0; i<128; i++)
	{
		...
		srv_colortab_hi[i] =  0xff000000 | (red<16) |  (grn<8) | blu ; // SDL_MapRGB(srv_screen->format, red, grn, blu);
		srv_colortab_med[i] = 0xff000000 | (((red*med)/100)<16) |  (((grn*med)/100)<8) | ((blu*med)/100) ; // SDL_MapRGB(srv_screen->format, (red*med)/100, (grn*med)/100, (blu*med)/100);
		srv_colortab_low[i] = 0xff000000 | (((red*hi)/100)<16) |  (((grn*hi)/100)<8) | ((blu*hi)/100) ; // SDL_MapRGB(srv_screen->format, (red*hi)/100,  (grn*hi)/100,  (blu*hi)/100);
	}
}
...
{
 	void (*copy)();
	dd bpp = 32; // srv_screen->format->BytesPerPixel;
	dd i;
